...
The [`FilesystemMiddleware`](/oss/javascript/deepagents/middleware) automatically evicts large tool results to the file system when they exceed a token threshold, preventing context window saturation.
...
See [backends](/oss/javascript/deepagents/backends) for configuration details and examples.
...
The harness has built-in streaming for real-time updates from agent execution.
