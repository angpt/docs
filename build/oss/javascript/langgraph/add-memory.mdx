...
<Tip>
You can also access the store using keyword argument injection: `def my_node(state, config: RunnableConfig, *, store: BaseStore)`. This pattern is useful if you don't need the full `Runtime` object or prefer the sync API. See [persistence](/oss/javascript/langgraph/persistence#memory-store) for more details on the store interface.
</Tip>
