...
The [`FilesystemMiddleware`](/oss/python/deepagents/middleware) automatically evicts large tool results to the file system when they exceed a token threshold, preventing context window saturation.
...
See [backends](/oss/python/deepagents/backends) for configuration details and examples.
